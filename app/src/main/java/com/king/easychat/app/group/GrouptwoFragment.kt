 ...
 */
class GrouptwoFragment : BaseFragment<GroupViewModel,GroupFragmentBinding>(),View.OnClickListener{

    val mAdapter by lazy { BindingAdapter<MessageNewBean>(R.layout.rv_group_item_two) }

    companion object{
        fun newInstance(): GrouptwoFragment{
            return GrouptwoFragment()
        }
    ...
    override fun initData(savedInstanceState: Bundle?) {
        tvTitle.setText(R.string.menu_group1)
        ...
        mViewModel.groupsLiveData.observe(this, Observer<List<Group>>{
            it?.let {
                ...
                mAdapter.replaceData(DateUtil.message)
                getApp().groups = it
            }
            srl.isRefreshing = false
        })
    ...
    fun clickItem(data: MessageNewBean){
        val intent = newIntent(data.groupName,NoticeActivity::class.java)
        intent.putExtra("title",data.groupName)
